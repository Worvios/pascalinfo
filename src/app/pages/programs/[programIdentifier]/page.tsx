...
import {
  ArrowRight, BookOpen, CalendarDays, CheckCircle, ListChecks, Users, Target,
  BookMarked, GraduationCap, Briefcase, BarChart3, Presentation, Languages,
  BarChartHorizontal, LucideIcon, HelpCircle, Award, BookText, School,
  Projector, CheckSquare, Globe,
} from "lucide-react";
...
const DetailSection = ({ title, icon: Icon, children }: { title: string; icon?: LucideIcon; children: React.ReactNode; }) => {
  ...
  return (
    <div className="pt-4 border-t border-border/10 first:border-t-0 first:pt-0">
      ...
      <div className="text-muted-foreground leading-relaxed space-y-2 pl-7">{content}</div>
    </div>
  );
};
    ...
    <div className="py-16 md:py-24 px-4 md:px-8 bg-gradient-to-b from-background via-background to-muted/10 relative">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-sm text-muted-foreground mb-6">
          ...
          <Link href="/pages/programs" className="hover:underline">{t("navigation.programs")}</Link>
          <span className="mx-2">&gt;</span>
        ...
        <div className="mb-10 md:mb-16 text-center">
          ...
          <p className="text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto">{program.description}</p>
        </div>
        ...
        <div className="text-center space-y-4">
          <Link href="/pages/contact" passHref>
            <Button size="lg" className="rounded-full gap-2 group shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
